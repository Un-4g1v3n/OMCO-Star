...
def deduplicate_csv(output_file):
    ...
    if "telemetryDate" in df.columns:
        ...
        df = df.sort_values("telemetryDate")
...
def main():
    ...
    print(f"Old files skipped: {skipped_old:,}")
    print(f"Previously processed skipped: {skipped_existing:,}")
    print(f"Unique site CSVs updated: {len(site_files):,}")
